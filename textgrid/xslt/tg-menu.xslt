...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:tgmenu="http://tgmenu" exclude-result-prefixes="xs tgmenu" version="2.0">
        ...
        <li>
            <a>
                <xsl:attribute name="href">
                    <xsl:choose>
                        <xsl:when test="@type = 'text/xml'">content.html?id=/xml/data/<xsl:value-of select="substring-after(@uri, ':')"/>.xml</xsl:when>
                        <xsl:when test="@type = 'text/linkeditorlinkedfile'">content.html?id=/xml/tile/<xsl:value-of select="substring-after(@uri, ':')"/>.xml</xsl:when>
    ...
    <xsl:template match="navigation">
        <div class="row">
            <div class="span3">
                <div id="nav-textgrid">
                    <ul class="nav nav-list nav-pills nav-stacked bs-sidebar affix">
                        <li>
                            <label class="tree-toggle nav-header">
                                <a href="index.html">
